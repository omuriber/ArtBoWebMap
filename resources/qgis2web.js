...
map.getView().fit([-8258357.435948, 513144.858129, -8231556.013477, 521932.807537], map.getSize());
...
var geolocateControl = (function (Control) {
    geolocateControl = function(opt_options) {
        ...
        spinner.className = 'fa fa-spinner geolocate-loading';
        ...
        var handleGeolocate = function() {
            if (isTracking) {
                ...
                // Espera a que se obtenga la posición
                var oncePosition = function() {
                  spinner.style.display = 'none';
                  button.disabled = false;
                  map.getView().setCenter(geolocation.getPosition());
                  isTracking = true;
                  geolocation.un('change:position', oncePosition);
                };
                geolocation.on('change:position', oncePosition);
            }
        };
...
golocationLayerSource = new ol.source.Vector({
  features: [accuracyFeature, positionFeature]
});
var geolocateOverlay = new ol.layer.Vector({
  source: golocationLayerSource
});

// Siempre mostrar capa de geolocalización
map.addLayer(geolocateOverlay);

// Seguimiento en tiempo real SIEMPRE
geolocation.setTracking(true);
geolocation.on('change:position', function() {
  var coordinates = geolocation.getPosition();
  positionFeature.setGeometry(coordinates ? new ol.geom.Point(coordinates) : null);
  if (coordinates && isTracking) {
    map.getView().setCenter(coordinates);
    // map.getView().setZoom(18); // Descomenta si quieres zoom automático
  }
});
